...
The local operator can create, edit, pause, and delete magnitude-threshold alerts; select email and/or Slack; and see recent earthquakes, alerts, and delivery results in an admin view. A scheduled poll ingests a documented USGS feed. A fixture-driven demo proves the full path without waiting for a live event. Setup instructions cover credentials and reproduction.
...
| `Alert` | ID, name, minimum magnitude (`double`), enabled, email enabled, Slack enabled, created UTC, updated UTC, optional deleted UTC. Deletion can hide an alert while retaining delivery history. |
...
| 16–20 h | Alert setup and admin view. `feat: manage alerts and inspect results` | Create/edit/pause/delete work; recent events and failed deliveries are visible; fixture demo is repeatable. |
